...
export abstract class AbstractMessageParser implements IMessageParser {
   ...
   */
  public createNormalizedMessageFromXML(
    xmlElement: Element,
    sourceMessage: ParsedMessage
  ): ParsedMessage {
   ...
   * Throws an error if normalized xml is not well formed.
   */
  createNormalizedMessageFromXMLString(
    xmlString: string,
    sourceMessage: ParsedMessage
  ): ParsedMessage {
    const doc: Document = new DOMParser().parseFromString(
      "<dummy>" + xmlString + "</dummy>",
      "text/xml"
    );
   ...
   */
  protected abstract processStartElement(
    elementNode: Element,
    message: ParsedMessage
  ): boolean;
   ...
   * @param message message to be altered
   */
  protected abstract processEndElement(
    elementNode: Element,
    message: ParsedMessage
  );
  ...
  public parseNormalizedString(
    ...
    try {
      ...
      throw new Error(
        format(
          'unexpected error while parsing message: "%s" (parsed "%")',
          error.message,
          normalizedString
        )
      );
    ...
    tokens.forEach((token: Token) => {
      ...
      switch (token.type) {
        ...
        case END_TAG:
          ...
          if (
            openTags.length === 0 ||
            openTags[openTags.length - 1] !== token.value.name
          ) {
            // oops, not well formed
            throw new Error(
              format(
                'unexpected close tag "%s" (parsed "%s")',
                token.value.name,
                normalizedString
              )
            );
        ...
        case PLACEHOLDER:
          disp = sourceMessage
            ? sourceMessage.getPlaceholderDisp(token.value.idcounter)
            : null;
          message.addPlaceholder(token.value.idcounter, disp);
          break;
        case ICU_MESSAGE_REF:
          disp = sourceMessage
            ? sourceMessage.getICUMessageRefDisp(token.value.idcounter)
            : null;
        ...
        case ICU_MESSAGE:
          throw new Error(
            format(
              '<ICUMessage/> not allowed here, use parseICUMessage instead (parsed "%")',
              normalizedString
            )
          );
        default:
  ...
  protected createXmlRepresentation(message: ParsedMessage): Element {
    const root: Document = new DOMParser().parseFromString(
      "<dummy/>",
      "text/xml"
    );
  ...
  protected abstract addXmlRepresentationToRoot(
    message: ParsedMessage,
    rootElem: Element
  );

  protected createXmlRepresentationOfTextPart(
    part: ParsedMessagePartText,
    rootElem: Element
  ): Node {
   ...
   */
  protected abstract createXmlRepresentationOfStartTagPart(
    part: ParsedMessagePartStartTag,
    rootElem: Element,
    id?: number
  ): Node;
   ...
   * @param rootElem rootElem
   */
  protected abstract createXmlRepresentationOfEndTagPart(
    part: ParsedMessagePartEndTag,
    rootElem: Element
  ): Node;
   ...
   * @param id id number in xliff2
   */
  protected abstract createXmlRepresentationOfEmptyTagPart(
    part: ParsedMessagePartEmptyTag,
    rootElem: Element,
    id?: number
  ): Node;
   ...
   * @param id id number in xliff2
   */
  protected abstract createXmlRepresentationOfPlaceholderPart(
    part: ParsedMessagePartPlaceholder,
    rootElem: Element,
    id?: number
  ): Node;
   ...
   * @param rootElem rootElem
   */
  protected abstract createXmlRepresentationOfICUMessageRefPart(
    part: ParsedMessagePartICUMessageRef,
    rootElem: Element
  ): Node;
}
