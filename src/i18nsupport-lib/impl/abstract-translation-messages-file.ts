...
export abstract class AbstractTranslationMessagesFile
  implements ITranslationMessagesFile
{
  ...
  protected parseContent(
    ...
    this._parsedDocument = new DOMParser().parseFromString(
      xmlString,
      "text/xml"
    );
   ...
   */
  abstract importNewTransUnit(
    foreignTransUnit: ITransUnit,
    isDefaultLang: boolean,
    copyContent: boolean,
    importAfterElement?: ITransUnit
  ): ITransUnit;
   ...
   * When false, content will be left empty (if it is not the default language).
   */
  abstract createTranslationFileForLang(
    lang: string,
    filename: string,
    isDefaultLang: boolean,
    copyContent: boolean
