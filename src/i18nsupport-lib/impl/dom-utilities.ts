import { DOMParser, XMLSerializer, Document, Element, Node } from "../xml-dom";
/**
...
export class DOMUtilities {
   ...
   */
  public static getFirstElementByTagName(
   ...
   * @return subelement or null, if not existing.
   */
  public static getElementByTagNameAndId(
    element: Element | Document,
    tagName: string,
    id: string
  ): Element {
  ...
  public static replaceContentWithXMLContent(element: Element, pcdata: string) {
    ...
    const pcdataFragment: Document = new DOMParser().parseFromString(
      "<fragment>" + pcdata + "</fragment>",
      "application/xml"
    );
    const newChildren = pcdataFragment
   ...
   * @return new element
   */
  public static createFollowingSibling(
    elementNameToCreate: string,
    previousSibling: Node
  ): Element {
    const newElement =
      previousSibling.ownerDocument.createElement(elementNameToCreate);
